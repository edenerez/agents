...
function NewAgent(domain, role, oppRole, gameid,country) {
  ...
  console.log("NewAgent", "default constractor");
...
NewAgent.prototype = {
  ...
  initializeNewAgent: function (){
    ...
    if (self.role == 'Employer'){
      console.log("==========the opponent role is employer==========")
      ...
      console.log("===========the opponent role is candidate==========")
    ...
    console.log("--- The posible Opponent at the begining is " + self.nik + " ---");
  },
  ...
  opponentRejected: function (offer, turn){
    ...
    if (typeof(offer) != "object"){
      ...
      self.gole = offer;
      offer = tempOffer;
      console.log(offer +" was rejected 258" );
    }
    ...
    console.log(self.searchCluster.length + " NUMBERS OF OFFER EXIST 288")
    if(!self.gole)
      self.gole = self.findMyNextIssueToTalk();
    self.removeFromSearchCluster(offer[self.gole]);// remove the current gole which the opponent rejected
    console.log(self.searchCluster.length + " NUMBERS OF OFFER LEFT 290")
    ...
    if (self.temp){
      ...
      logs.writeJsonLogGame(self.gameid, self.status, "BASED ON THAT BID (line 336) ", self.temp);
      logs.writeJsonLogGame(self.gameid, self.status, "============================ ", "");
      ...
      if (self.temp){//if it find a value with the current term add the offer to B
        ...
        logs.writeJsonLogGame(self.gameid, self.status, "BASED ON THAT BID (line 361) ", self.temp);
        logs.writeJsonLogGame(self.gameid, self.status, "============================ ", "");
  ...
  checkBid: function (offer, turn){
    ...
    if (len ==  self.issuesLength) {// check if the bid is full.
      ...
      console.log("OPPONENT UTILITY ++++++++++++++++ "+ currUtil +" ++++++++++++++++ 363")
      console.log("MY UTILITY ++++++++++++++++ "+ myUtility +" ++++++++++++++++ 364")
      ...
      console.log(" 375 CURRENT OPPONENT " +self.posibleOpponent[self.currOpponent].nikName);
      ...
      if(self.searchCluster.length < MIN_OFFERS_IN_SEARCH_CLUSTER){
        logs.writeJsonLogGame(self.gameid, self.status, "offer in the search cluster: ", self.searchCluster.length);
        self.recalculateSearchCluster(self.turn);
        console.log("recalculate the searchCluster ")
        logs.writeJsonLogGame(self.gameid, self.status, "offer in the search cluster: ", self.searchCluster.length);
      }
      ...
      if (self.temp){
        ...
        logs.writeJsonLogGame(self.gameid, self.status, "BASED ON THAT BID (line 447) ", self.temp);
        logs.writeJsonLogGame(self.gameid, self.status, "============================ ", "");
        ...
        if (self.temp){
          ...
          logs.writeJsonLogGame(self.gameid, self.status, "BASED ON THAT BID (line 496) ", self.temp);
          logs.writeJsonLogGame(self.gameid, self.status, "============================ ", "");
          
          self.B = clone(tempb);
          console.log("offer is rejected and offer something else 458")
          ...
          if (self.temp){ //if it find a value with the current term
            ...
            logs.writeJsonLogGame(self.gameid, self.status, "BASED ON THAT BID (line 521) ", self.temp);
            logs.writeJsonLogGame(self.gameid, self.status, "============================ ", "");
            self.searchCluster = clone(tempSearchCluster);  
        ...
        if (self.B.length == 0 && !self.temp){
          ...
          logs.writeJsonLogGame(self.gameid, self.status, "BASED ON THAT BID (line 530) ", self.temp);
          logs.writeJsonLogGame(self.gameid, self.status, "============================ ", "");
          self.B = clone(tempb);
          self.B_temp = clone(tempb);
          self.searchCluster = clone(tempSearchCluster);
  ...
  checkOpponent: function(turn, offer){
    ...
    var updatedTypeProbability = 0;
      ...
      for (var i = 0; i < self.posibleOpponent.length; i++){
       ...
       self.currOpponent  = 0;
       for (var i = 0; i < self.posibleOpponent.length; i++){
        console.log(self.posibleOpponent[i].nikName + "557");
        console.log(self.posibleOpponent[i].probability + "558");
        if (self.posibleOpponent[i].probability > self.posibleOpponent[self.currOpponent].probability){
          self.currOpponent = i;
          self.nik = self.posibleOpponent[i].nikName;
        }
  ...
  tryToCompromise: function(turn){
    ...
    console.log(self.B_Rely)
    console.dir(self.initBids[bidNum].utilMe)
    console.log(self.myUtilityShort.timeeffect)
    console.log('turn --- ' + turn);
    var currUtil = self.initBids[bidNum].utilMe + (turn*self.myUtilityShort.timeeffect);
    console.log('currUtil ' + currUtil +" ************* bidNum " + bidNum);
  ...
  findValueMinMax: function (min){
    ...
    for (bid in self.searchCluster){
      ...
      console.log('maxUtility ' + maxUtility);
      console.log('minUtility ' + minUtility);
