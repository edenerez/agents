...
//new
var turnBeforOffer = 2 //HOW LONG DO WE WAIT FOR A RESPONSE AFTER SENING AN OFFER
var waitForOpponentFirstOffer = false;
var opponentRejectedMyLastOfferOrMadeCounterOffer = false;
var lastTurnOfOffer = -100; // negative so the agent will offer on the first turn
...
function KBAgent(domain, role, oppRole,gameid,country) {
  ...
  //this.initializeKBAgent();
  //this.initializeBids(domain);
  console.log("KBAgent", "default constractor");
...
KBAgent.prototype = {
  ...
  pickBid: function (turn){
      ...
      console.log("############################################");
      console.log(this.posibleOpponent[this.currOpponent].agentAcceptThersholds[turn]);
      console.log("===========================");
 ...
 /* makeFullOffer: function(offer){
  ...
  /*checkOpponent: function(turn, offer){
    ...
    var self = this;
    var sumProbabilities = 0;
    var bigestProbability = 0;  

      for (var i = 0; i < self.posibleOpponent.length; i++){ 
        var name = self.posibleOpponent[i].nikName; //the nik name of current opponent
        self.posibleOpponent[i].oppUtility = self['oppUtility'+name].getUtilityWithDiscount(offer['utilOpp'+name], turn); //calculate the utility with discount.
        var luc = self.posibleOpponent[i].oppUtility / self['sumUtilOpp'+name] //calculate the luc number of the current offer
        sumProbabilities += self.posibleOpponent[i].calculateCurrOpp(luc); // add the numerator of the calculation of the probability
      } 
      for (var i = 0; i < self.posibleOpponent.length; i++){
        self.posibleOpponent[i].probability = self.posibleOpponent[i].calcProbability / sumProbabilities; //the new probability
        console.log(self.posibleOpponent[i].nikName);
        console.log(self.posibleOpponent[i].probability);
        if (self.posibleOpponent[i].probability > bigestProbability){

          bigestProbability = self.posibleOpponent[i].probability;
          self.currOpponent = i;
          self.nik = self.posibleOpponent[i].nikName;
          console.log (self.posibleOpponent[i].nikName);
        }
