...
var domains = {};
var country1;
exports.AgentManagertest = function(country){
	switch(country){
	...
	async.series([
	    function(callback){
	        if (agentType == 'KBAgent'){
	        	...
	        	async.series([
				    function(callback){
				        agent = new KBAgent(domains['Job'], role, opponentRole, gameid+"test",country1);
			...
			if (agentType == 'NegoChatAgent'){
				...
				async.series([
				    function(callback){
				        agent = new NegoChatAgent(domains['Job'], role, opponentRole, gameid+"test",country1);
			...
			if (agentType == 'ChatAgent'){
				...
				async.series([
				    function(callback){
				        agent = new ChatAgent(domains['Job'], role, opponentRole, gameid+"test",country1);
				        callback(null, 'two.one');
				    },
				    function(callback){
			...
			if (agentType == 'NewAgent'){
				...
				async.series([
				    function(callback){
				        agent = new NewAgent(domains['Job'], role, opponentRole, gameid+"test",country1);
				        callback(null, 'two.one');
				    },
				    function(callback){
